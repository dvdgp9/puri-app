...
## Planner: Reorganización Index + Sistema de Favoritos

### Background and Motivation
Reorganizar `index.php` para añadir sistema de favoritos sin comprometer espacio. Mover logo y saludo al header superior, añadir sección de favoritos debajo del hero, e implementar modal de acceso con contraseña para favoritos.

### Key Changes
1. **Header superior**: Logo + "Hola, soy Puri" en una línea horizontal
2. **Sección favoritos**: Debajo del hero, máximo 3 favoritos con acceso directo
3. **Modal de acceso**: Al clicar favorito, modal con info de actividad + campo contraseña
4. **Base de datos**: Nueva tabla `favoritos` para persistir favoritos por dispositivo
...
CREATE TABLE favoritos (
    ...
    UNIQUE KEY unique_device_activity (device_id, actividad_id)
);
```

### High-level Task Breakdown
1. **Reorganización UI index.php**
   - Crear header superior con logo + saludo horizontal
   - Añadir sección favoritos debajo del hero
   - Mantener formulario de selección de centro existente

2. **Sistema de favoritos**
   - Crear tabla `favoritos` en BD
   - Implementar device_id único por navegador (localStorage)
   - API endpoints para CRUD de favoritos
   - UI para mostrar favoritos en index

3. **Modal de acceso a favoritos**
   - Modal con info de actividad (centro, instalación, actividad)
   - Campo de contraseña del centro
   - Botón acceder que valida y redirige a asistencia.php

4. **Integración en asistencia.php**
   - Botón estrella para añadir/quitar de favoritos
   - Feedback visual del estado de favorito

### Success Criteria
- Index reorganizado con header superior y sección favoritos
- Máximo 3 favoritos persistentes por dispositivo
- Modal de acceso funcional con validación de contraseña
- Integración completa en flujo de asistencia

### Project Status Board (Favoritos)
- [ ] Crear tabla `favoritos` en BD
- [ ] Reorganizar layout de index.php (header + sección favoritos)
- [ ] Implementar device_id y localStorage
- [ ] Crear API endpoints para favoritos
- [ ] Implementar modal de acceso con contraseña
- [ ] Añadir botón estrella en asistencia.php
- [ ] Integrar feedback visual y toasts