...
function setupParticipantModal() {
    ...
    if (manualForm) {
        manualForm.addEventListener('submit', async function(e) {
            e.preventDefault();
            await createMultipleParticipants();
        });
    }
...
function addParticipantRow() {
    ...
    row.innerHTML = `
        <td>
            <input type="text" class="form-input participant-nombre" placeholder="Nombre" 
                   data-row="${rowIndex}" onkeydown="handleParticipantKeyDown(event)">
        </td>
        <td>
            <input type="text" class="form-input participant-apellidos" placeholder="Apellidos" 
                   data-row="${rowIndex}" onkeydown="handleParticipantKeyDown(event)">
        </td>
        <td>
            <button type="button" class="btn-remove-row" onclick="removeParticipantRow(this)" 
                    title="Eliminar fila">
    ...
    const firstInput = row.querySelector('.participant-nombre');
    if (firstInput) {
...
function handleParticipantKeyDown(event) {
    ...
    if (event.key === 'Enter') {
        ...
        if (input.classList.contains('participant-nombre')) {
            // Mover a apellidos de la misma fila
            const apellidosInput = row.querySelector('.participant-apellidos');
            if (apellidosInput) apellidosInput.focus();
        } else if (input.classList.contains('participant-apellidos')) {
            // Mover a nombre de la siguiente fila o crear nueva
            const nextRow = row.nextElementSibling;
            if (nextRow) {
                const nextNombreInput = nextRow.querySelector('.participant-nombre');
                if (nextNombreInput) nextNombreInput.focus();
            } else {
                // Añadir nueva fila si ambos campos están llenos
                const nombreInput = row.querySelector('.participant-nombre');
    ...
    // Tab: comportamiento normal del navegador
 ...
 */
async function createMultipleParticipants() {
    const btn = document.getElementById('createParticipantBtn');
    const btnText = btn.querySelector('.btn-text');
    const btnLoading = btn.querySelector('.btn-loading');
    ...
    try {
        ...
        const participantes = collectParticipantsData();
        
        if (participantes.length === 0) {
            showFieldError('participantsTable', 'Debe añadir al menos un participante');
            return;
        }
