<?php
// Endpoint temporal para comparar con el oficial
session_start();
...
try {
    ...
    // Filtro para administradores (no superadmin)
    $centro_filter = '';
    $params = [];
    
    if ($admin_info['role'] !== 'superadmin') {
        $centro_filter = ' AND c.id IN (
            SELECT centro_id FROM admin_asignaciones 
            WHERE admin_id = :admin_id
        )';
        $params['admin_id'] = $admin_info['id'];
    }
    ...
    $stmt->execute($params);
    $stats['total_centros'] = (int)$stmt->fetchColumn();
    ...
    $stmt = $pdo->prepare($query);
    $stmt->execute($params);
    $stats['total_instalaciones'] = (int)$stmt->fetchColumn();
    ...
    $stmt = $pdo->prepare($query);
    $stmt->execute($params);
    $stats['total_actividades'] = (int)$stmt->fetchColumn();
    ...
    // Top centros con más actividades
    $query = "
        SELECT 
            c.nombre,
    ...
    error_log("Error de BD en test_stats: " . $e->getMessage());
    echo json_encode([
        'success' => false,
        'error' => 'Error de base de datos: ' . $e->getMessage()
    ]);
} catch (Exception $e) {
    error_log("Error en test_stats: " . $e->getMessage());
    echo json_encode([
        'success' => false,
